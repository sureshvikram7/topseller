...
public class ProductPageController extends AbstractPageController
{
	...
	@Resource(name = "classificationClassesResolverStrategy")
			...
			throws CMSItemNotFoundException, UnsupportedEncodingException
	{
		...
		testClassification(productModel);
	...
	public void testClassification(ProductModel productModel1) {

		final FlexibleSearchQuery fQuery = new FlexibleSearchQuery("select {pk} from {product} where {pk} not in({{SELECT {product} FROM {ProductFeature} WHERE  {classificationAttributeAssignment}  IN (8796095316578) and {stringvalue} is NOT NULL}})");
		...
		for (ProductModel productModel : productModelList) {
			List<ClassAttributeAssignmentModel> list = getAssignmentsForProduct(productModel);
			if (productModel instanceof GenericVariantProductModel) {
				GenericVariantProductModel genericVariantProductModel = (GenericVariantProductModel) productModel;

				for (ClassAttributeAssignmentModel classAttributeAssignmentModel : list) {
					final ProductFeatureModel productFeature = modelService.create(ProductFeatureModel.class);
					productFeature.setProduct(productModel);
					productFeature.setValue(genericVariantProductModel.getBaseProduct().getCode());
					productFeature.setLanguage(commonI18NService.getCurrentLanguage());
					productFeature.setClassificationAttributeAssignment(classAttributeAssignmentModel);
					productFeature.setValuePosition(classAttributeAssignmentModel.getPosition());
				...
				for (ClassAttributeAssignmentModel classAttributeAssignmentModel : list) {
					final ProductFeatureModel productFeature = modelService.create(ProductFeatureModel.class);
					productFeature.setProduct(productModel);
					productFeature.setValue(productModel.getCode());
					productFeature.setLanguage(commonI18NService.getCurrentLanguage());
					productFeature.setClassificationAttributeAssignment(classAttributeAssignmentModel);
					productFeature.setValuePosition(classAttributeAssignmentModel.getPosition());
					modelService.save(productFeature);
	...
	{
		final Set<ClassificationClassModel> classificationClasses = classificationClassesResolverStrategy.resolve(product);
		return classificationClassesResolverStrategy.getAllClassAttributeAssignments(classificationClasses);
	}
